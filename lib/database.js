...
module.exports = {
  ...
  get_distribution: function(richlist, stats, cb) {
    var distribution = {
      supply: stats.supply,
      t_1_10: {percent: 0, total: 0 },
      t_11_100: {percent: 0, total: 0 },
      t_101_500: {percent: 0, total: 0 },
      t_501_1000: {percent: 0, total: 0 },
      t_1001plus: {percent: 0, total: 0 }
    };
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      if (count <= 10 ) {
        distribution.t_1_10.percent = distribution.t_1_10.percent + percentage;
        distribution.t_1_10.total = distribution.t_1_10.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 100 && count > 10) {
        distribution.t_11_100.percent = distribution.t_11_100.percent + percentage;
        distribution.t_11_100.total = distribution.t_11_100.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 500 && count > 100) {
        distribution.t_101_500.percent = distribution.t_101_500.percent + percentage;
        distribution.t_101_500.total = distribution.t_101_500.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 1000 && count > 500) {
        distribution.t_501_1000.percent = distribution.t_501_1000.percent + percentage;
        distribution.t_501_1000.total = distribution.t_501_1000.total + (richlist.balance[i].balance / 100000000);
      }

      loop.next();
    }, function() {
      distribution.t_1001plus.percent = parseFloat(100 - distribution.t_501_1000.percent - distribution.t_101_500.percent - distribution.t_11_100.percent - distribution.t_1_10.percent - (settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? ((richlist.burned / 100000000) / stats.supply) * 100 : 0)).toFixed(2);
      distribution.t_1001plus.total = parseFloat(distribution.supply - distribution.t_501_1000.total - distribution.t_101_500.total - distribution.t_11_100.total - distribution.t_1_10.total - (settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? (richlist.burned / 100000000) : 0)).toFixed(8);
      distribution.t_1_10.percent = parseFloat(distribution.t_1_10.percent).toFixed(2);
      distribution.t_1_10.total = parseFloat(distribution.t_1_10.total).toFixed(8);
      distribution.t_11_100.percent = parseFloat(distribution.t_11_100.percent).toFixed(2);
      distribution.t_11_100.total = parseFloat(distribution.t_11_100.total).toFixed(8);
      distribution.t_101_500.percent = parseFloat(distribution.t_101_500.percent).toFixed(2);
      distribution.t_101_500.total = parseFloat(distribution.t_101_500.total).toFixed(8);
      distribution.t_501_1000.percent = parseFloat(distribution.t_501_1000.percent).toFixed(2);
      distribution.t_501_1000.total = parseFloat(distribution.t_501_1000.total).toFixed(8);
